...
Semantic vector search (Node) on the managed database, which ships with pgvector enabled; a placeholder embedding is built in and `embed.js` shows where a real model goes. Deploy target: Dockhold (https://dockhold.eu).
...
2. `npx dockhold deploy --name pgvector-starter --db` from this folder. `--db` adds the managed database this app needs; Dockhold injects `DATABASE_URL`.
...
- The Deploy button link in README.md opens the Dockhold dashboard with the repo and name pre-filled; the user signs in, confirms, and the app builds: https://app.dockhold.eu/new?repo=https://github.com/dockhold/pgvector-starter&name=pgvector-starter&ref=button
...
- `DIM` in `embed.js` is a code constant, not an environment variable. Change it to match the embedding model if the user swaps one in.
